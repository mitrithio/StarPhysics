...
PCEMatrix<ROW, COL>::PCEMatrix() 
	...
	, m_uiColumns( COL )
{
	m_mMatrix = new float[ ROW * COL ]();
}

template<unsigned int ROW, unsigned int COL>
PCEMatrix<ROW,COL>::PCEMatrix( const float i_args[], unsigned int i_uiCount /*= ROW*COL*/ )
	: m_uiRows( ROW )
	, m_uiColumns( COL )
{
	m_mMatrix = new float[ROW*COL]();
...
template<unsigned int ROW, unsigned int COL>
PCEMatrix<ROW,COL>& PCEMatrix<ROW,COL>::operator*=( float i_fScalar )
{
	for ( unsigned int uiRow = 0; uiRow < ROW; ++uiRow )
	{
...
template<unsigned int ROW, unsigned int COL>
float* PCEMatrix<ROW,COL>::operator[]( unsigned int i_uiRow ) const
{
...
PCEMatrix<N,N>::PCEMatrix()
	...
	, m_uiRows(N)
{
	m_mMatrix = new float[N*N]();
}

template<unsigned int N>
PCEMatrix<N,N>::PCEMatrix( const float i_args[], unsigned int i_uiCount /*= N*N*/ )
	: m_uiRows( N )
	, m_uiColumns( N )
{
	m_mMatrix = new float[N*N]();
...
template<unsigned int N>
PCEMatrix<N,N>& PCEMatrix<N,N>::operator*=( float i_fScalar )
{
	for ( unsigned int uiRow = 0; uiRow < N; ++uiRow )
	{
...
template<unsigned int N>
float* PCEMatrix<N,N>::operator[]( unsigned int i_uiRow ) const
{
...
PCEMatrix<N,1>::PCEMatrix()
	...
	, m_uiColumns(1)
{
	m_mMatrix = new float[N];
}

template<unsigned int N>
PCEMatrix<N,1>::PCEMatrix( const float i_args[], unsigned int i_uiCount /*= N*/ )
	: m_uiRows( N )
	, m_uiColumns( 1 )
{
	m_mMatrix = new float[N]();
...
template<unsigned int N>
PCEMatrix<N,1>& PCEMatrix<N,1>::operator*=( float i_fScalar )
{
	for ( unsigned int uiIndex = 0; uiIndex < N; ++uiIndex )
	{
...
template<unsigned int N>
PCEMatrix<N,1> PCEMatrix<N,1>::operator*( float i_fScalar ) const
{
	PCEMatrix<N,1> oVector = *this;
	oVector *= i_fScalar;
	return oVector;
...
template<unsigned int N>
float& PCEMatrix<N,1>::operator[]( unsigned int i_uiIndex ) const
{
	return m_mMatrix[i_uiIndex];
}

template<unsigned int N>
float PCEMatrix<N,1>::Module() const
{
	float fModuleModule = 0;
	for ( unsigned int uiIndex = 0; uiIndex < N; ++uiIndex )
	{
...
template<unsigned int N>
PCEMatrix<1,N>::PCEMatrix( const float i_args[], unsigned int i_uiCount /*= N*/ )
	: m_uiRows( 1 )
	, m_uiColumns( N )
{
	m_mMatrix = new float[N]();
...
template<unsigned int N>
PCEMatrix<1,N>& PCEMatrix<1,N>::operator*=( float i_fScalar )
{
	for ( unsigned int uiIndex = 0; uiIndex < N; ++uiIndex )
	{
		m_mMatrix[uiIndex] *= i_fScalar;
...
template<unsigned int N>
PCEMatrix<1,N> PCEMatrix<1,N>::operator*( float i_fScalar ) const
{
	PCEMatrix<1,N> oVector = *this;
	oVector *= i_fScalar;
	return oVector;
...
template<unsigned int N>
float PCEMatrix<1,N>::operator*( const PCEMatrix<N,1>& i_other ) const
{
	float fRes = 0;
	for ( unsigned int uiIndex = 0; uiIndex < N; ++uiIndex )
	{
...
template<unsigned int N>
float& PCEMatrix<1,N>::operator[]( unsigned int i_uiIndex ) const
{
	return m_mMatrix[i_uiIndex];
}

template<unsigned int N>
float PCEMatrix<1,N>::Module() const
{
	float fModuleModule = 0;
	for ( unsigned int uiIndex = 0; uiIndex < N; ++uiIndex )
	{
